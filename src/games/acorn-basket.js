...
export const gameAcornBasket = (app, username, handleScoreUpdate) => {
  ...
  async function game_init() {
    ...
    await Assets.init({ manifest: assetLibrary });
    ...
    // Objects
    const food = [];
    const foodAssets = await Assets.loadBundle('game-screen');
    const foodTypes = [
    ...
    function spawnFood() {
      ...
      try {
        ...
        foodObj.anchor.set(0.5);
        foodObj.x = getRandomNumber(80, (app.renderer.width - 170));
        foodObj.y = 0;
        if(type.type !== 'bomb') {
          foodObj.rotation = getRandomNumber(-2,2);
        }
